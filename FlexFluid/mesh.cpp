...
static inline Point3F SafeNormalize(const Point3F& v, const Point3F& fallback=Point3F())
{
    float l = v.lengthSquared();
    if (l > 0.0f)
    {
        return v * (1.f / sqrtf(l));
    }
    else
        return fallback;
}
...
void Mesh::CalculateNormals()
{
    ...
    for (int i=0; i < numTris; ++i)
    {
        ...
        Vector3F n = Vector3F::crossProduct(m_positions[b]-m_positions[a], m_positions[c]-m_positions[a]);
...
void Mesh::Rotate(float angle, const Vector3F& axis)
{
    Matrix4 rotator;
    rotator.rotate(angle, axis);
...
Mesh* Mesh::ImportMesh(const char* path)
{
...
Mesh* Mesh::ImportMeshFromBin(const char* path)
{
    Timer timer;
    ...
    if (f)
    {
        ...
        len = fread(&m->m_positions[0], sizeof(Vector3F)*numVertices, 1, f);
        len = fread(&m->m_normals[0], sizeof(Vector3F)*numVertices, 1, f);
        ...
        printf("Imported mesh %s in %d ms\n", path, Timer::toMsecs(timer.release()));
...
void Mesh::ExportMeshToBin(const char* path) const
{
    ...
    if (f)
    {
        ...
        // write data blocks
        fwrite(&m->m_positions[0], sizeof(Vector3F)*numVertices, 1, f);
        fwrite(&m->m_normals[0], sizeof(Vector3F)*numVertices, 1, f);
...
Mesh* Mesh::ImportMeshFromPly(const char* path)
{
    ...
    mesh->m_colours.resize(numVertices, Color3F(1.0f, 1.0f, 1.0f));
    ...
    for (uint32_t v=0; v < numVertices; ++v)
    {
        ...
        mesh->m_positions[v] = Point3F(properties[0], properties[1], properties[2]);
        mesh->m_normals[v] = Vector3F(0.0f, 0.0f, 0.0f);
    }
    ...
    for (uint32_t f=0; f < numFaces; ++f)
    {
        ...
        default:
            Q_ASSERT(!"invalid number of indices, only support tris and quads");
            break;
        };

        // calculate vertex normals as we go
        Point3F& v0 = mesh->m_positions[indices[0]];
        Point3F& v1 = mesh->m_positions[indices[1]];
        Point3F& v2 = mesh->m_positions[indices[2]];

        Vector3F n = SafeNormalize(Vector3F::crossProduct(v1-v0, v2-v0), {0.0f, 1.0f, 0.0f});
    ...
    {
        mesh->m_normals[i] = SafeNormalize(mesh->m_normals[i], {0.0f, 1.0f, 0.0f});
    }
...
Mesh* Mesh::ImportMeshFromObj(const char* path)
{
    ifstream file(path);
    ...
    while (file)
    {
        ...
        else if (buffer[0] == 'v')
        {
            ...
            positions.push_back(Point3F(x, y, z));
        }
            ...
            for (int i=0; i < 4; ++i)
            {
                ...
                if (!file.eof())
                {
                    ...
                    else
                    {
                        ...
                        m->m_colours.push_back(Color3F(1.0f, 1.0f, 1.0f));
    ...
    for (uint32_t i=0; i < numFaces; ++i)
    {
        ...
        Point3F& v0 = m->m_positions[a];
        Point3F& v1 = m->m_positions[b];
        Point3F& v2 = m->m_positions[c];
    ...
    {
        m->m_normals[i] = SafeNormalize(m->m_normals[i], {0.0f, 1.0f, 0.0f});
    }
...
void Mesh::ExportMeshToObj(const char* path) const
{
    ofstream file(path);
    const Mesh& m = *this;
...
void Mesh::GetBounds(Vector3F& outMinExtents, Vector3F& outMaxExtents) const
{
    Point3F minExtents(FLT_MAX,FLT_MAX,FLT_MAX);
    Point3F maxExtents(-FLT_MAX,-FLT_MAX,-FLT_MAX);

    // calculate face bounds
    for (uint32_t i=0; i < m_positions.size(); ++i)
    {
        const Point3F& a = m_positions[i];

        minExtents = Min(a, minExtents);
        maxExtents = Max(a, maxExtents);
    }

    outMinExtents = Vector3F(minExtents);
    outMaxExtents = Vector3F(maxExtents);
}

Mesh* Mesh::CreateTriMesh(float size, float y)
{
    uint32_t indices[] = { 0, 1, 2 };
    Point3F positions[3];
    Vector3F normals[3];

    positions[0] = Point3F(-size, y, size);
    positions[1] = Point3F(size, y, size);
    positions[2] = Point3F(size, y, -size);
...
Mesh* Mesh::CreateQuadMesh(float size, float y)
{
    ...
    positions[1] = Point3F(size, y, size);
    positions[2] = Point3F(size, y, -size);
    positions[3] = Point3F(-size, y, -size);
...
Mesh* Mesh::CreateDiscMesh(float radius, uint32_t segments)
{
    ...
    m->m_positions[0] = Point3F(0.0f, 0.f, 0.f);
    m->m_positions[1] = Point3F(0.0f, 0.0f, radius);
    ...
    for (uint32_t i=1; i <= segments; ++i)
    {
        ...
        m->m_positions[nextVert] = Point3F(radius*sin((float(i)/segments) * 2*M_PI), 0.0f, radius*cos((float(i)/segments) * 2*M_PI));
        m->m_normals[nextVert] = Vector3F(0.0f, 1.0f, 0.0f);
...
Mesh* Mesh::CreateTetrahedron(float ground, float height)
{
    Mesh* m = new Mesh();

    const float dimValue = 1.0f / sqrtf(2.0f);
    const Point3F vertices[4] =
    {
        Point3F(-1.0f, ground, -dimValue),
        Point3F(1.0f, ground, -dimValue),
        Point3F(0.0f, ground + height, dimValue),
        Point3F(0.0f, ground, dimValue)
    };
...
Mesh* Mesh::CreateSphere(int slices, int segments, float radius)
{
    ...
    for (int i = 0; i <= slices; ++i)
    {
        ...
        for (int j = 0; j <= segments; ++j)
        {
            ...
            mesh->m_positions.push_back(Point3F(x, y, z)*radius);
            mesh->m_normals.push_back(Vector3F(x, y, z));
...
Mesh* Mesh::CreateCapsule(int slices, int segments, float radius, float halfHeight)
{
    float dTheta = M_PI / (slices * 2);
    float dPhi = 2*M_PI / segments;
    ...
    for (int i = 0; i <= 2 * slices + 1; ++i)
    {
        for (int j = 0; j <= segments; ++j)
        {
            ...
            mesh->m_positions.push_back(Point3F(x, y, z)*radius + Vector3F(0.0f, yoffset, 0.0f));
            mesh->m_normals.push_back(Vector3F(x, y, z));
