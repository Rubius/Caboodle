...
public:
    ...
    AABBTree(const Vector3F* vertices, uint32_t numVerts, const uint32_t* indices, uint32_t numFaces);
    ...
    bool TraceRaySlow(const Vector3F& start, const Vector3F& dir, float& outT, float& u, float& v, float& w, float& faceSign, uint32_t& faceIndex) const;
    bool TraceRay(const Vector3F& start, const Vector3F& dir, float& outT, float& u, float& v, float& w, float& faceSign, uint32_t& faceIndex) const;

    void DebugDraw();
    
    Vector3F GetCenter() const { return (m_nodes[0].m_minExtents+m_nodes[0].m_maxExtents)*0.5f; }
    const Vector3F& GetMinExtents() const { return m_nodes[0].m_minExtents; }
    const Vector3F& GetMaxExtents() const { return m_nodes[0].m_maxExtents; }
...
private:
    ...
    struct Bounds
    {
        ...
        {
            m_min = AABBTree::VMin(m_min, b.m_min);
            m_max = AABBTree::VMax(m_max, b.m_max);
        }
    ...
    void TraceRecursive(uint32_t nodeIndex, const Vector3F& start, const Vector3F& dir, float& outT, float& u, float& v, float& w, float& faceSign, uint32_t& faceIndex) const;
 
    void CalculateFaceBounds(uint32_t* faces, uint32_t numFaces, Vector3F& outMinExtents, Vector3F& outMaxExtents);
